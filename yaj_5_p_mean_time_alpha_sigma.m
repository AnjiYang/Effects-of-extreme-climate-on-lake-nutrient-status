...
for j = 2:N
    ...
    if alpha(ii) ~= 1 
        V = unifrnd(-pi/2,pi/2);
        W = exprnd(1);
        const = beta*tan(pi*alpha(ii)/2);
        B = atan( const );
        S = (1 + const * const).^(1/(2*alpha(ii)));
        dL(j) = ( S * sin( alpha(ii)*V + B ) ./ ( cos(V) ).^(1/alpha(ii)) .* ...
            ( cos( (1-alpha(ii)) * V - B ) ./ W ).^((1-alpha(ii))/alpha(ii)) )*alpha_1;
